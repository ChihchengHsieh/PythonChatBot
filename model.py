...
def weights_init(m):
    classname = m.__class__.__name__ # python trick that will look for the type of connection in the object "m" 
    ...
    if classname.find('Conv') != -1: # if the connection is a convolution
        ...
        m.weight.data.uniform_(-w_bound, w_bound) # generating some random weights of order inversely proportional to the 
                                                  # size of the tensor of weights
        ...
        w_bound = np.sqrt(6. / (fan_in + fan_out)) # weight bound
        m.weight.data.uniform_(-w_bound, w_bound) # generating some random weights of order inversely proportional to 
                                                  # the size of the tensor of weights
        m.bias.data.fill_(0) # initializing all the bias with zeros
...
class ActorCritic(torch.nn.Module):
    ...
    def __init__(self, num_inputs, action_space):
        ...
        self.lstm = nn.LSTMCell(32 * 3 * 3, 256) # making an LSTM (Long Short Term Memory) to learn the temporal properties of the 
                                                 #input - we obtain a big encoded vector S of size 256 that encodes an event of the game
    ...
    # returning the output of the critic (V(S)), the output of the actor (Q(S,A)), and the new hidden & cell states ((hx, cx))
