...
class SharedAdam(optim.Adam): # object that inherits from optim.Adam
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, weight_decay=0):
        ...
        for group in self.param_groups:
            # self.param_groups contains all the attributes of the optimizer, 
            #including the parameters to optimize (the weights of the network) 
            # contained in self.param_groups['params']
            for p in group['params']: # for each tensor p of weights to optimize
                state = self.state[p] 
                # at the beginning, self.state is an empty dictionary so state = {} and self.state = {p:{}} = {p: state}
